...
{
	local first_option all_short_options='adlnpxtvceh'
	first_option="${1:0:1}"
...
print_help()
{
	printf '%s\n' "boomi [ATOM | MOLECULE | APIM] --add --name NAME --token TOKEN [--path PATH] [--vm VM_OPTIONS --container CONTAINER_OPTIONS]
boomi [ATOM | MOLECULE | APIM] --delete --name NAME
boomi ADDON --add --name NAME [--port PORT] [--path PATH] [--node NODEPORT]
boomi ADDON --delete --name NAME
boomi ADDON --list"
	printf 'Usage: %s [-a|--(no-)add] [-d|--(no-)delete] [-l|--(no-)list] [-n|--name <arg>] [-p|--path <arg>] [-x|--port <arg>] [-t|--token <arg>] [-v|--vm <arg>] [-c|--container <arg>] [-e|--node <arg>] [-h|--help] [<operation>]\n' "$0"
